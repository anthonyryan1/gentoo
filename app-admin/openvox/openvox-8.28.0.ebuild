...
inherit eapi9-ver ruby-fakegem systemd tmpfiles
...
all_ruby_install() {
	...
	# agent systemd unit
	systemd_newunit "${WORKDIR}/all/${P}/ext/systemd/puppet.service" puppet.service
...
pkg_postinst() {
	...
	if ver_replacing -eq 8.27.0; then
		ewarn "The service has been reverted back to 'puppet'"
		ewarn "If you already migrated to the 'openvox' service name, revert that change:"
		ewarn "  OpenRC:  rc-update del openvox; rc-update add puppet"
		ewarn "  systemd: systemctl disable openvox; systemctl enable puppet"
		ewarn "Our apologies for the inconvenience."
	fi
