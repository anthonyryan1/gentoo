...
IUSE="aio debug +http +http2 http3 +http-cache ktls libatomic mail pcre +pcre2 pcre-jit rtmp selinux ssl stream threads vim-syntax"
...
for mod in $NGINX_MODULES_STD; do
	...
	REQUIRED_USE="${REQUIRED_USE} nginx_modules_http_${mod}? ( http )"
done
...
for mod in $NGINX_MODULES_STREAM_STD; do
	...
	REQUIRED_USE="${REQUIRED_USE} nginx_modules_stream_${mod}? ( stream )"
done
...
for mod in $NGINX_MODULES_MAIL; do
	...
	REQUIRED_USE="${REQUIRED_USE} nginx_modules_mail_${mod}? ( mail )"
done

for mod in $NGINX_MODULES_3RD; do
	IUSE="${IUSE} nginx_modules_${mod}"
	if [[ $mod == http_* ]]; then
		REQUIRED_USE="${REQUIRED_USE} nginx_modules_${mod}? ( http )"
	fi

	if [[ $mod == stream_* ]]; then
		REQUIRED_USE="${REQUIRED_USE} nginx_modules_${mod}? ( stream )"
	fi
done
...
src_configure() {
	...
	# third-party modules
	use nginx_modules_http_upload_progress      && myconf+=( --add-module=${HTTP_UPLOAD_PROGRESS_MODULE_WD} )
	use nginx_modules_http_headers_more         && myconf+=( --add-module=${HTTP_HEADERS_MORE_MODULE_WD} )
	use nginx_modules_http_cache_purge          && myconf+=( --add-module=${HTTP_CACHE_PURGE_MODULE_WD} )
	use nginx_modules_http_slowfs_cache         && myconf+=( --add-module=${HTTP_SLOWFS_CACHE_MODULE_WD} )
	use nginx_modules_http_fancyindex           && myconf+=( --add-module=${HTTP_FANCYINDEX_MODULE_WD} )
	...
	use nginx_modules_http_auth_pam             && myconf+=( --add-module=${HTTP_AUTH_PAM_MODULE_WD} )
	use nginx_modules_http_upstream_check       && myconf+=( --add-module=${HTTP_UPSTREAM_CHECK_MODULE_WD} )
	use nginx_modules_http_metrics              && myconf+=( --add-module=${HTTP_METRICS_MODULE_WD} )
	use nginx_modules_http_naxsi                && myconf+=( --add-module=${HTTP_NAXSI_MODULE_WD} )
	use rtmp                                    && myconf+=( --add-module=${RTMP_MODULE_WD} )
	use nginx_modules_http_dav_ext              && myconf+=( --add-module=${HTTP_DAV_EXT_MODULE_WD} )
	use nginx_modules_http_echo                 && myconf+=( --add-module=${HTTP_ECHO_MODULE_WD} )
	use nginx_modules_http_security             && myconf+=( --add-module=${HTTP_SECURITY_MODULE_WD} )
	use nginx_modules_http_push_stream          && myconf+=( --add-module=${HTTP_PUSH_STREAM_MODULE_WD} )
	use nginx_modules_http_sticky               && myconf+=( --add-module=${HTTP_STICKY_MODULE_WD} )
	use nginx_modules_http_mogilefs             && myconf+=( --add-module=${HTTP_MOGILEFS_MODULE_WD} )
	use nginx_modules_http_memc                 && myconf+=( --add-module=${HTTP_MEMC_MODULE_WD} )
	use nginx_modules_http_auth_ldap            && myconf+=( --add-module=${HTTP_LDAP_MODULE_WD} )
	use nginx_modules_http_vhost_traffic_status && myconf+=( --add-module=${HTTP_VHOST_TRAFFIC_STATUS_MODULE_WD} )
	...
	use nginx_modules_http_brotli               && myconf+=( --add-module=${HTTP_BROTLI_MODULE_WD} )

	# Stream modules
	for mod in $NGINX_MODULES_STREAM_STD; do
		use nginx_modules_stream_${mod} || myconf+=( --without-stream_${mod}_module )
	done

	for mod in $NGINX_MODULES_STREAM_OPT; do
		use nginx_modules_stream_${mod} && myconf+=( --with-stream_${mod}_module )
	done

	# MAIL modules
	for mod in $NGINX_MODULES_MAIL; do
		use nginx_modules_mail_${mod} || myconf+=( --without-mail_${mod}_module )
	done
